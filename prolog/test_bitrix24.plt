...
:- begin_tests(install, [setup((
                               wrap_predicate(bitrix24_request:get(Url, Reply, Options), install,
                                             _Wrapped, mock_get_install(Url, Reply, Options)),
                                   assert(bitrix24_config:config(event, onCrmAdd, 'http://localhost/rest')),
                                   assert(bitrix24_config:config(placement, 'LIST_MENU', 'http://localhost/rest')))),
                        cleanup((
                                 unwrap_predicate(bitrix24_request:get(Url, Reply, Options), install),
                                 retract(bitrix24_config:config(event, _, _)),
                                 retract(bitrix24_config:config(placement, _, _))
                                 ))]).
...
bitrix24_request:mock_get_install('https://localhost/rest/event.get?auth=4d9636640061cc84002c4d40000000010000078db61a43f077dd35398b6201f7795438',
                                  [], [status_code(200)]) :- !.

bitrix24_request:mock_get_install('https://localhost/rest/placement.get?auth=4d9636640061cc84002c4d40000000010000078db61a43f077dd35398b6201f7795438',
                                  [], [status_code(200)]) :- !.

bitrix24_request:mock_get_install(_Url, Reply, [status_code(200)]) :-
    Reply = [result = @(true)].
